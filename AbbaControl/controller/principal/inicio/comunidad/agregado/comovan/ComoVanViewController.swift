...
class ComoVanViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func apiSolicitarDatos(){
        ...
        let encodeURL = apiListadoPlanesComunidaYoAddComoVan
                    ...
                    case .success(let data):
                        ...
                        if let successValue = json["success"].int {
                            ...
                            if(successValue == 1){
                                ...
                                self.itemTotalPlan = json["itemtotal"].int ?? 0
                                
                                json["listadoplan"].array?.forEach({ (dataArray) in
                                                   
                                    let nombre = dataArray["nombre"].string ?? ""
                                    let correo = dataArray["correo"].string ?? ""
                                    let conteo = dataArray["conteo"].int ?? 0
                                    
                                                                          
                                    let informacion = ModeloItemYoAgregue(nombre: nombre, correo: correo, conteo: conteo)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell1", for: indexPath) as! ComoVanTableViewCell
        ...
        let textoDevoTotal = TextoIdiomaController.localizedString(forKey: "devocional_total")
        let textoDevoRealizado = TextoIdiomaController.localizedString(forKey: "devocional_realizados")
        
    
        cell.txtNombre.numberOfLines = 0
        cell.txtCorreo.numberOfLines = 0
        cell.txtDevoTotal.numberOfLines = 0
        cell.txtNombre.numberOfLines = 0

        cell.txtNombre.text = textoNombre + " " + selectedItem.nombre
        cell.txtCorreo.text = textoCorreo + " " + selectedItem.correo
        cell.txtDevoTotal.text = textoDevoTotal + ": " + String(itemTotalPlan)
        cell.txtDevoRealizado.text = textoDevoRealizado + ": " + String(selectedItem.conteo)
